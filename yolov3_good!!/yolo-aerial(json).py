...
ap.add_argument("-a", "--address", required=True, help="IP address of main server")
...
#sender = imagezmq.ImageSender(connect_to='tcp://'+args["address"]+':8887')
HOST = args["address"]
PORT = 8887
BUFSIZE = 1024
...
try:
    ...
    while True:
        ...
        print("carNum : ", carNum)
        print("Error!")
        file_data["hor_up"] = carNum
        file_data["hor_bottom"] = carNum
        file_data["ver_left"] = carNum
        file_data["ver_right"] = carNum
        print("Error!")
        sendData = json.dumps(file_data, ensure_ascii=False)
        clientSocket.sendall(bytes(sendData, 'UTF-8'))
        print(sendData)
